...
class ListShowtimes extends ListRecords
{
    ...
    {
        return [
            // ปุ่มสร้างรอบฉายอัตโนมัติตามช่วงเวลา
            Action::make('generateShowtimes')
                ->label('สร้างรอบฉายอัตโนมัติ')
                ->color('success')
                ->icon('heroicon-o-arrow-path')
                ->modalHeading('สร้างรอบฉายอัตโนมัติตามตารางประจำสัปดาห์ (Weekly Schedules)')
                ->modalDescription('ระบบจะดึงรอบฉายแม่แบบประจำสัปดาห์มาสร้างรอบฉายจริงตามช่วงวันที่กำหนด โดยจะไม่สร้างซ้ำกับรอบที่มีอยู่แล้ว')
                ->modalSubmitActionLabel('เริ่มสร้างรอบฉาย')
                ->form([
                    Select::make('range_type')
                        ->label('เลือกช่วงเวลาที่ต้องการสร้าง')
                        ->options([
                            '7_days' => '7 วันข้างหน้า (Next 7 Days)',
                            '14_days' => '14 วันข้างหน้า (Next 14 Days)',
                            'end_of_month' => 'จนถึงสิ้นเดือนนี้ (End of this month)',
                            'next_month' => 'ตลอดทั้งเดือนหน้า (Full Next Month)',
                            'custom' => 'กำหนดช่วงวันที่เอง (Custom Range)',
                        ])
                        ->default('14_days')
                        ->reactive()
                        ->required(),
