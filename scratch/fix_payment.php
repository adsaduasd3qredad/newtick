...
// Find the QR Code div
$content = preg_replace('/<div class="flex justify-center mb-10">.*?{!! QrCode::size.*?<\/div>\s*<\/div>\s*<\/div>/s', '', $content);
            ...
            <div class="flex justify-center max-w-md mx-auto">
                <form method="POST" action="{{ route('bookings.confirm', \$booking->id) }}" class="w-full">
                    ...
                    <button type="submit"
                        ...
                        <span>รับรหัสตั๋ว (E-Ticket)</span>
...
$content = preg_replace('/<div class="flex flex-col sm:flex-row gap-4 justify-center max-w-md mx-auto">.*?<\/form>\s*<\/div>/s', $newForms, $content);

file_put_contents($file, $content);
echo "Payment view updated successfully.\n";
